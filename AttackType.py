...
features = [
    ...
    'latitude', 'longitude', 'specificity', 'targtype1_txt', 
    'targsubtype1_txt', 'weaptype1_txt', 'weapsubtype1_txt', 'nkill', 'nwound', 
    'property', 'ishostkid', 'INT_LOG', 'INT_IDEO', 'INT_MISC', 'INT_ANY'
]

# Filter the dataset to include only selected features and the target
df = df[features + ['attacktype1_txt']]
...
categorical_features = ['country_txt', 'region_txt', 'targtype1_txt', 
...
X = df.drop(columns=['attacktype1_txt'])
y = df['attacktype1_txt']

# Encode the target variable (y) into numeric values
label_encoder = LabelEncoder()
y = label_encoder.fit_transform(y)  # Converts attack types into numeric labels
...
xgb_model = XGBClassifier(n_estimators=100, random_state=42, use_label_encoder=False, eval_metric='mlogloss')  # Use 'mlogloss' for multi-class
